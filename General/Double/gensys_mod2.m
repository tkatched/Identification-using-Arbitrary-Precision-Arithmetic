function [G1,C,impact,ywt,fmat,fwt,loose,gev,eu]=gensys_mod2(g0,g1,c,psi,pi,div) 
%This is a slight modification of Sims' matlab program following the development 
%in Lubik and Schorfheide (2003,2004) to allow for indeterminacy.
...
if ~zxz
  ...
  %It moves all the eigenvalues that are selected by 'ordering' vector to
  %upper left corner, hence the unstable one goes to bottom right. Final
  %order same as qzdiv and Gauss.
...
neta = size(pi,2); %number of forecast errors
if nunstab == 0
  etawt == zeros(0,neta);
  ueta = zeros(0,0);
  deta = zeros(0,0);
...
[ueta,deta,veta,veta2,R]=svdrr(etawt);
end
...
if eu==[1;1]
loose = G0I * [etawt1 * (eye(neta) - veta * veta');zeros(nunstab, neta)]; % TETA under determinacy - Sims
elseif eu(1)==1 && eu(2)==0 && isempty(veta2)==0
loose=G0I * [etawt1 * (eye(neta) - veta * veta')*veta2;zeros(nunstab, size(veta2,2))]; %TETA augmented for indeterminacy
elseif eu(1)==1 && eu(2)==0 && isempty(veta2)==1
   ...
   %return
   loose=G0I * [etawt1 * (eye(neta) - veta * veta');zeros(nunstab, size(veta2,2))]; %changed to handle indeterminacy due to no unstable roots
else
