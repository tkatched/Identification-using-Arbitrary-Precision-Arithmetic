function G = gmatsgul_mpv2(theta0,x,wg,h,dif)

%Function to compute local identification condition for the SGU (2012) model at
%a point theta0 in arbitrary precision, using partially parallelized
%computation.
...
%uses auxiliary files sgusolvl_mp.m, gensys_mp2.m, svdrr_mp.m,
%sgusspar_mp.m, sgueqvarshocksl.m, sgusetsysmatl_mp.m
...
[TT,TC,TEPS,TETA,RC] = sgusolvl_mp(theta0);
...
if dif==1 %use symmetric Newton's quotient
    ...
    for j=1:ntheta;
        ...
        if j<14 %for parameters affecting gensys solution, resolve the model
            [nT1,nTC,nTEPS,nTETA,nRC] = sgusolvl_mp(npara);
            [pT1,pTC,pTEPS,pTETA,pRC] = sgusolvl_mp(ppara);
        ...
        parfor i=1:nfreq;
            ...
            tempp=(A*pmat1*pRR*pQQ*pRR'*pmat2*A')/(pi2); %spectra at perturbed theta0
    ...
    for j=1:ntheta;
        ...
        if j<14 %for parameters affecting gensys solution, resolve the model
            [nT1,nTC,nTEPS,nTETA,nRC] = sgusolvl_mp(npara);
            [pT1,pTC,pTEPS,pTETA,pRC] = sgusolvl_mp(ppara);
            [n2T1,n2TC,n2TEPS,n2TETA,n2RC] = sgusolvl_mp(n2para);
            [p2T1,p2TC,p2TEPS,p2TETA,p2RC] = sgusolvl_mp(p2para);
        ...
        parfor i=1:nfreq;
            ...
            tempn=(A*nmat1*nRR*nQQ*nRR'*nmat2*A')/(pi2); %spectra at perturbed theta0
            tempn2=(A*n2mat1*n2RR*n2QQ*n2RR'*n2mat2*A')/(pi2); %spectra at perturbed theta0
            tempp=(A*pmat1*pRR*pQQ*pRR'*pmat2*A')/(pi2); %spectra at perturbed theta0
            tempp2=(A*p2mat1*p2RR*p2QQ*p2RR'*p2mat2*A')/(pi2); %spectra at perturbed theta0
