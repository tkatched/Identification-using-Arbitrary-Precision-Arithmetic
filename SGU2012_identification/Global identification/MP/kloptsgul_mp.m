function X = kloptsgul_mp(thetainput,thetab,thetaa,truespec,x,wg,bc,parsel,con,wgt,nrm,indp,lb,ub)

%Function to compute the KL distance between SGU (2012) models (observables
%in levels) in arbitrary precision
...
if con(1)==1
    conval=constraintsgu(thetat,thetab,thetaa,parsel,con(2),wgt,nrm,indp);
...
[TT,TC,TEPS,TETA,RC] = sgusolvl_mp(theta0); %solve model using Sims algorithm
...
QQ=mp(zeros(21,21));
for i=14:34
    QQ(i-13,i-13)=(theta0(i)/hhmp)^twop; %put variances on the diagonal (dividing by 100 in HS(2014) code)
...
for i=1:nx %compute spectrum
    ...
    spec=(A*mat1*RR*QQ*RR'*mat2*A')/(pi2); %compute spectrum using representation in the paper
