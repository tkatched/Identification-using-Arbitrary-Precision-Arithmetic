function KL = kllp_mp(theta1,theta2,msel1,msel2,n,bc)
%computes KL distance between parameterizations (theta1,theta2) of the 
%Leeper(1991) type models

%Inputs:

% theta1 - benchmark parameter vector (must be an MP object)
% theta2 -alternative parameter vector (must be an MP object)
...
%solve both models
[TT1,TC1,TEPS1,TETA1,RC1] =  lpsolv_mp2(theta1,msel1); %solve model using Sims algorithm
[TT2,TC2,TEPS2,TETA2,RC2] =  lpsolv_mp2(theta2,msel2);
...
A1=[mp(eye(ny)),mp(zeros(ny,neq1-ny))]; %selection matrix A1
A2=[mp(eye(ny)),mp(zeros(ny,neq2-ny))]; %selection matrix A2

QQ1=createcov_lpmp(theta1,msel1,RC1);
QQ2=createcov_lpmp(theta2,msel2,RC2);
...
for i=1:nx %compute spectrum
    ...
    mat11=(mp(eye(neq1))-TT1*exe)\mp(eye(neq1));
    ...
    mat12=(mp(eye(neq2))-TT2*exe)\mp(eye(neq2));
    ...
    temp=spec2\spec1;
    KL=KL+mp('2')*(trace(temp) - log(det(temp))-ny)*wg(i);
...
KL=real(KL)*(mp('1/4/pi')); %final answer: KL. real() necessary since
