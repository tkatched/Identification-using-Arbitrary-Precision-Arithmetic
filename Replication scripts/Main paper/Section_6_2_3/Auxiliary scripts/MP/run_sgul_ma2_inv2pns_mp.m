%% Quadruple precision check of the closest model with (unanticipated) MA(2) shocks parameterized as Y_t=theta1*e_t+theta2*e_t-1
...
elseif bc==1
    resfilename=['Sgul_ma2_inv2pnsnb_bc'];
end
...
[TT,TC,TEPS,TETA,RC] = sgusolvl3_mp(thetab,msel1);
...
QQ=mp(zeros(21,21));
for i=14:34
    QQ(i-13,i-13)=(thetab(i))^twop; %put variances on the diagonal (dividing by 100 in HS(2014) code)
...
lb=[mp('[0.001 ,7E-06,0.01,0.01,0.00001,-0.99,-0.99,-0.99,-0.99,-0.99,-0.99,-0.99,-0.99]'),[kron(mp(ones(1,7)),mp('[0.01,-7,-5]'))]];
ub=[mp('[15,0.999,35,15,0.999,0.99,0.99,0.99,0.99,0.99,0.99,0.99,0.99]'),[kron(mp(ones(1,7)),mp('[20,7,5]'))]];
...
ObjFunmp=@(theta0)kloptsgulinv2_mp(theta0,thetab,thetaa,truespec,x,wg,bc,parsel,con2,wgt,nrm,indp,lb,ub);
