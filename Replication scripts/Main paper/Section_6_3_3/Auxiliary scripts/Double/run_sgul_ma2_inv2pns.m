...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        ub=[15,0.999,35,15,0.999,0.99,0.99,0.99,0.99,0.99,0.99,0.99,0.99,kron(ones(1,7),[20,7,5])];
        lb=lb(parsel);
        ub=ub(parsel);
        ...
        if bc==0
            resfilename=['Sgul_ma2_inv2pns'];
        elseif bc==1
            resfilename=['Sgul_ma2_inv2pns_bc'];
        end
        ...
        if runpso==1
            ...
            psoname=['psosgul_ma2_inv2pns','_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
