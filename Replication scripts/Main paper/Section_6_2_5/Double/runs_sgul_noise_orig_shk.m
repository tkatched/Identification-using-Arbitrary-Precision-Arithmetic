%This script performs search for observationally equivalent points for the
 %news specification of the noise shock version of the SGU (2012) model as in
 %Chahrour and Jurado (2018).
 
 
 %Successful optimization should recover a parameter value very close to thetab
        %% Setup
        ...
        msel1=2; %benchmark model - noise shocks
        
        msel2=1; %search over news shock models
        %% Base parameter values for target news regime:
        ...
        thetab=[thet;gam;kap;del2_del1;b;rhoxg;rhoa;rhox;rhozi;rhoz;rhom;rhog;rhozet;sig_mua0;sig_mua4;
            ...
            sig_g0; sig_g4;sig_g8;sig_zet0;sig_zet4;sig_zet8;sig_me]'; %base parameter vector for target regime
        ...
        [gx,hx,eta,npara] = sgusolvn(thetab,msel1);
...
for i=1:length(x) %compute spectrum under noise
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*gx2*mat1*eta*eta'*mat2*gx2'*A')/(2*pi); %compute spectrum using representation in the paper
     
end

thetabnoise=[thetab(1:13),npara]; %collect original noise shock parameters into benchmark theta0              
        ...
        lb=[0.001 ,7E-06,0.01,0.01,0.00001,-0.999,-0.999,-0.999,-0.999,-0.999,-0.999,-0.999,-0.999,zeros(1,22)];
        ub=[15,0.999,35,15,0.999,0.999,0.999,0.999,0.999,0.999,0.999,0.999,0.999,ones(1,22)*20];
        lb=lb(parsel);
        ub=ub(parsel);
        ...
        ObjectiveFunction = @(theta0)kloptsguln(theta0,thetaa,truespec,msel2,x,wg,bc,parsel,con1,wgt,nrm,indp); %set objective function for GA/multistart
        ObjectiveFunctionP = @(theta0)kloptsguln(theta0,thetaa,truespec,msel2,x,wg,bc,parsel,con2,wgt,nrm,indp); %set objective function for PSO
        ...
        if runpso==1
            ...
            psoname=['psosgul_noiseorig_shk']; %set name for a temp output file that stores the swarms (problem-based)
