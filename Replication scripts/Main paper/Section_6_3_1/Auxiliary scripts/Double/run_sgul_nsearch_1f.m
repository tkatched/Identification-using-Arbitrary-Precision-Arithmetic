...
%with the weakest identified parameter (sigma8_mu) fixed.

%Specify parameters for search:
parstruc(1,1).ind=[[1:27],[29:34]]; % 1 weakly id. parameters fixed at theta0 values
ns=[0.1;0.5;1]; %sizes of excluded neighborhoods

for kk=1:1
    for jj=1:3
        clearvars -except jj kk ii parstruc ns ppool
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        ub=[15,0.999,35,15,0.999,0.99,0.99,0.99,0.99,0.99,0.99,0.99,0.99,ones(1,22)*20];
        lb=lb(parsel);
        ub=ub(parsel);
        ...
        if bc==0
            resfilename=[['Sgul_bch_kl_1f'],'_ns',num2str(ns(jj)*10)];
        elseif bc==1
            resfilename=[['Sgul_bch_kl_1f_bc'],'_ns',num2str(ns(jj)*10)];
        end
        ...
        if runpso==1
            ...
            psoname=[['psosgul_bch_1f'],['ns'],num2str(ns(jj)*10),'_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
