...
%with the 7 weakest identified parameters fixed.
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        elseif bc==1
            resfilename=[['Sgul_bch_kl_7f_bc'],'_ns',num2str(ns(jj)*10)];
        end
        ...
        if runpso==1
            ...
            psoname=[['psosgul_bch_7f'],['ns'],num2str(ns(jj)*10),'_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
