...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        if bc==0
            resfilename=['Sgul_nonews_mua_kl'];
        elseif bc==1
            resfilename=['Sgul_nonews_mua_kl_bc'];
        end
        ...
        if runpso==1
            ...
            psoname=['psosgul_nonews_mua_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
