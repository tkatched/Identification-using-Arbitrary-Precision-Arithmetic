%% Global identification of the SGU (2012) model with all the 8-period news shocks shut down.

        %% Setup
        
        
        parsel=[1:34]; 
        parsel([16:3:34])=[]; %all 8-period news shock variances set to zero
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        if bc==0
            resfilename=['Sgul_nonews8_kl'];
        elseif bc==1
            resfilename=['Sgul_nonews8_kl_bc'];
        end
        ...
        if runpso==1
            ...
            psoname=['psosgul_nonews8_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
