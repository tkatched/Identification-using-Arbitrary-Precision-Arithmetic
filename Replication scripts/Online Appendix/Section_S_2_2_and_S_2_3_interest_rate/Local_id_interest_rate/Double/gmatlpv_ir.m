function G = gmatlpv_ir(theta0,x,wg,h,dif,msel)
...
A(3,1)=theta0(1); %add third observable, interest rate: R_t=alpha*pi_t+theta_t
...
if dif==1 %use symmetric Newton's quotient
    ...
    for j=1:ntheta;
        ...
        for i=1:nfreq;
            ...
            tempn=nA*nmat1*nRR*nQQ*nRR'*nmat2*nA'/(2*pi);
            tempp=pA*pmat1*pRR*pQQ*pRR'*pmat2*pA'/(2*pi);
    ...
    for j=1:ntheta;
        ...
        for i=1:nfreq;
            ...
            tempn=nA*nmat1*nRR*nQQ*nRR'*nmat2*nA'/(2*pi); %spectra at perturbed theta0
            tempn2=n2A*n2mat1*n2RR*n2QQ*n2RR'*n2mat2*n2A'/(2*pi);
            tempp=pA*pmat1*pRR*pQQ*pRR'*pmat2*pA'/(2*pi);
            tempp2=p2A*p2mat1*p2RR*p2QQ*p2RR'*p2mat2*p2A'/(2*pi);
            nsp_mat(((i-1)*dimy2+1):(i*dimy2),j)=tempn(:); %spectra at perturbed theta0
