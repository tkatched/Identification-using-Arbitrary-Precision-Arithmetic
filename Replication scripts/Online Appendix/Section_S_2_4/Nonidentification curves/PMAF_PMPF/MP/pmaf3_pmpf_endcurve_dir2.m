%This script verifies that values on the nonidentification curve in Table 5 result
%in observational equivalence (Direction 2)

%Specify parameters for search:
parstruc(1,1).ind=[1:7,9]; %modify nonzero parameters only
...
%         mp.Digits(pl); %impose precision level
        ...
        resfilename2=[resfilename,'_mp_pmaf3_cdir2_',num2str(jj)];
...
%         if jj<10
        load('curve_pmaf3_dir2','thetacheck')
        
        thetamp=mp(thetacheck(jj,parstruc(1,1).ind));
%         else
%             load('curve_ampf_dir2','theta2')
%             thetamp=mp(theta2(25540,parstruc(1,1).ind));
%         end
