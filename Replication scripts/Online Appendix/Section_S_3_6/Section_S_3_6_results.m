%Global identification of the SGU (2012) model with neighborhood size
%c=0.1, 0.5, 1 scaled by lengths of the 90% posterior intervals.

%Specify parameters for search:
parstruc(1,1).ind=[1:34]; %all parameters

ns=[0.1;0.5;1]; %sizes of excluded neighborhoods

for kk=1:1
    for jj=1:3
        clearvars -except jj kk ii parstruc ns
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        lb=[0.001 ,7E-06,0.01,0.01,0.00001,-0.999,-0.999,-0.999,-0.999,-0.999,-0.999,-0.999,-0.999,zeros(1,21)];
        ub=[15,0.999,35,15,0.999,0.999,0.999,0.999,0.999,0.999,0.999,0.999,0.999,ones(1,21)*20];
        lb=lb(parsel);
        ub=ub(parsel);
        
        numpar=length(lb);%number of parameters in the objective function
        
        %Posterior interval upper and lower bounds
        ui=[5.83,0.01,10.91,0.50,0.93,0.88,0.58,0.49,0.64,0.96,1,0.99,0.30,0.35,0.34,0.33,0.57,0.28,0.27,14.94,6.15,10.58,0.74,0.31,0.27,1.24,5.7,2.85,1.07,1.07,1,6.02,4.84,4.85];
        li=[3.88,0.0001,7.41,0.23,0.89,0.44,0.38,0.12,0.22,0.85,0.95,0.93,0.07,0.02,0.01,0.01,0.22,0.01,0.01,8.9,0.16,1.71,0.54,0.01,0.01,0.04,3.18,0.04,0.06,0.04,0.03,1.2,0.17,0.14];
        
        %% Detailed specifications; you should not have to modify them
        
        con1=[0;ns(jj)]; %constraint indicator for objective function used by GA and fmincon
        con2=[1;ns(jj)]; %constraint indicator for objective function used by PSO
        
        %Specify neighborhood and parameter constraint characteristics
        
        nrm=Inf; %set norm for the constraint function (1,2, or Inf)
        indp=parsel; %vector of parameter indices to be constrained
        wgt=ui(indp)-li(indp); %vector of weights for the constraint function
        
        %Specify constraint function:
        
        ConstraintFunction=@(xest)constraintsgu(xest,thetab,thetaa,parsel,con2(2),wgt,nrm,indp); %set constraint for the problem
        
        
        if bc==0
            resfilename=[['Sgul_wg_kl'],'_ns',num2str(ns(jj)*10)];
        elseif bc==1
            resfilename=[['Sgul_wg_kl_bc'],'_ns',num2str(ns(jj)*10)];
        end
        ...
        if runpso==1
            ...
            psoname=[['psosgul_wg_'],['ns'],num2str(ns(jj)*10)]; %set name for a temp output file that stores the swarms (problem-based)
        ...
        kl=fvalpso2/10000;
        ed80=pfhsgul2(thetab,xestpso2,0.05,80,n,bc);
        ed150=pfhsgul2(thetab,xestpso2,0.05,150,n,bc);
        ed200=pfhsgul2(thetab,xestpso2,0.05,200,n,bc);
        ed1000=pfhsgul2(thetab,xestpso2,0.05,1000,n,bc);
