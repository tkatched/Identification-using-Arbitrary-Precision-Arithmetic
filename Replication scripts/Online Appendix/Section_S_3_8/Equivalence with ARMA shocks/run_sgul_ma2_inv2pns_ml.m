%% Searches for the closest model with (unanticipated) MA(2) shocks parameterized as Y_t=theta1*e_t+theta2*e_t-1
        %% Setup
        ...
        [TT,TC,TEPS,TETA,RC] = sgusolvl3(thetab,msel1);
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        lb=[0.001 ,7E-06,0.01,0.01,0.00001,-0.99,-0.99,-0.99,-0.99,-0.99,-0.99,-0.99,-0.99,kron(ones(1,7),[0.01,-7,-5])];
        ub=[15,0.999,35,15,0.999,0.99,0.99,0.99,0.99,0.99,0.99,0.99,0.99,kron(ones(1,7),[50,7,5])];
        lb=lb(parsel);
        ub=ub(parsel);
        ...
        if bc==0
            resfilename=['Sgul_ml_ma2_inv2pns'];
        elseif bc==1
            resfilename=['Sgul_ml_ma2_inv2pns_bc'];
        end
        
        ObjectiveFunction = @(theta0)kloptsgulinv2(theta0,thetab,thetaa,truespec,x,wg,bc,parsel,con1,wgt,nrm,indp); %set objective function for GA/multistart
        ObjectiveFunctionP = @(theta0)kloptsgulinv2(theta0,thetab,thetaa,truespec,x,wg,bc,parsel,con2,wgt,nrm,indp); %set objective function for PSO
        ...
        if runpso==1
            ...
            psoname=['psosgul_ml_ma2_inv2pns','_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
        ...
        kl=fvalpso2/10000;
        ed80=pfhsgulma2ns(thetab,xestpso2,msel1,msel2,0.05,80,n,bc);
        ed150=pfhsgulma2ns(thetab,xestpso2,msel1,msel2,0.05,150,n,bc);
        ed200=pfhsgulma2ns(thetab,xestpso2,msel1,msel2,0.05,200,n,bc);
        ed1000=pfhsgulma2ns(thetab,xestpso2,msel1,msel2,0.05,1000,n,bc);
