%% Search for the closest model with the MA wage markup shock
...
%         ppool=parpool('local',numcore); %create parallel pool object
        ...
        ub=[15,0.999,35,15,0.999,0.99,0.99,0.99,0.99,0.99,0.99,0.99,0.99,ones(1,21)*50];
        
        lb(intsel)=[0,-7,-5]; %NB: lower bound relaxed
        ub(intsel)=[20,7,5];
        lb=lb(parsel);
        ub=ub(parsel);
        ...
        if bc==0
            resfilename=['Sgul_ml_ma2_msel_',num2str(msel2)];
        elseif bc==1
            resfilename=['Sgul_ml_ma2_msel_',num2str(msel2),'_bc'];
        end
        ...
        if runpso==1
            ...
            psoname=['psosgul_ml_ma2_msel_',num2str(msel2),'_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
        ...
        kl=fvalpso2/10000;
        ed80=pfhsgulma3ns(thetab,xestpso2,msel1,msel2,0.05,80,n,bc);
        ed150=pfhsgulma3ns(thetab,xestpso2,msel1,msel2,0.05,150,n,bc);
        ed200=pfhsgulma3ns(thetab,xestpso2,msel1,msel2,0.05,200,n,bc);
        ed1000=pfhsgulma3ns(thetab,xestpso2,msel1,msel2,0.05,1000,n,bc);
