...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        if bc==0
            resfilename=[['Sgul_ml_kl'],'_ns',num2str(ns(jj)*10)];
        elseif bc==1
            resfilename=[['Sgul_ml_kl_bc'],'_ns',num2str(ns(jj)*10)];
        end
        ...
        if runpso==1
            ...
            psoname=[['psosgul_ml_'],['ns'],num2str(ns(jj)*10)]; %set name for a temp output file that stores the swarms (problem-based)
