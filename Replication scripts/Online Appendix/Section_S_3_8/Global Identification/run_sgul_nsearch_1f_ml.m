%Global identification of the SGU (2012) model with neighborhood size c=0.1, 0.5, 1

%weakly identified parameter sig_zet_4 fixed

%Default parameter is the ML estimates from Table II SGU (2012)

%Specify parameters for search:
parstruc(1,1).ind=[[1:32],34]; % 1 weakly id. parameters fixed at theta0 values
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        ub=[15,0.999,35,15,0.999,0.999,0.999,0.999,0.999,0.999,0.999,0.999,0.999,ones(1,21)*50];
        lb=lb(parsel);
        ub=ub(parsel);
        ...
        if bc==0
            resfilename=[['Sgul_ml_kl1f'],'_ns',num2str(ns(jj)*10)];
        elseif bc==1
            resfilename=[['Sgul_ml_kl1f_bc'],'_ns',num2str(ns(jj)*10)];
        end
        ...
        if runpso==1
            ...
            psoname=[['psosgul1f_ml_'],['ns'],num2str(ns(jj)*10)]; %set name for a temp output file that stores the swarms (problem-based)
