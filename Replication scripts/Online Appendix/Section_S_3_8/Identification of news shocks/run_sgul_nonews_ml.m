%% Global identification of the SGU (2012) model with all the news shocks shut down.


        %% Setup
        
        
        parsel=[[1:13],[14:3:34]]; %all news shock variances set to zero
        ...
        thetaa=zeros(1,length(thetab));
        thetaa(parsel)=thetab(parsel);
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        ub=[15,0.999,35,15,0.999,0.999,0.999,0.999,0.999,0.999,0.999,0.999,0.999,ones(1,22)*50];
        lb=lb(parsel);
        ub=ub(parsel);
        ...
        if bc==0
            resfilename=['Sgul_ml_nonews_kl'];
        elseif bc==1
            resfilename=['Sgul_ml_nonews_kl_bc'];
        end
        ...
        if runpso==1
            ...
            psoname=['psosgul_ml_nonews']; %set name for a temp output file that stores the swarms (problem-based)
