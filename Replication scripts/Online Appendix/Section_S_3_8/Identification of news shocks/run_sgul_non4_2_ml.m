%% Global identification of the SGU (2012) model with the 4-period news in the nonstationary neutral productivity shock shut down.

        %% Setup
        
        
        parsel=[1:34]; %mux news shock variances set to zero
        parsel(18)=[];
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        if bc==0
            resfilename=['Sgul_ml_non4_2'];
        elseif bc==1
            resfilename=['Sgul_ml_non4_2_bc'];
        end
        ...
        if runpso==1
            ...
            psoname=['psosgul_ml_non4_2_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
