%% Global identification of the SGU (2012) model with the 8-period news in the stationary investment-specific productivity shock shut down.
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        if bc==0
            resfilename=['Sgul_ml_non8_3'];
        elseif bc==1
            resfilename=['Sgul_ml_non8_3_bc'];
        end
        ...
        if runpso==1
            ...
            psoname=['psosgul_ml_non8_3_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
