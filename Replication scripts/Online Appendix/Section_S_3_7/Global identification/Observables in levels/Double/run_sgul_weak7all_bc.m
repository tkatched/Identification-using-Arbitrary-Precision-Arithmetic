%Searches at theta0 with expanding neighborhoods
        ...
        parsel=[1:34]; %7 weakly id. news shock variances set to zero
        parsel([20,21,22,26,28,33,34])=[];
...
for i=1:length(x) %compute spectrum
    ...
    truespec(((i-1)*ny+1):i*ny,:)=(A*mat1*RR*QQ*RR'*mat2*A')/(2*pi); %compute spectrum using representation in the paper
        ...
        if bc==0
            resfilename=['Sgul_weak7all'];
        elseif bc==1
            resfilename=['Sgul_weak7all_bc'];
        end
        ...
        if runpso==1
            ...
            psoname=['psosgul_weak7all_',num2str(bc)]; %set name for a temp output file that stores the swarms (problem-based)
